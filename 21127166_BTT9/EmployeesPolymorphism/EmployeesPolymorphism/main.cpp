...
int main()
{
    ...
    for (auto employee : employees) {
        int num = employee->salary();
        Integer integer(num);
        IValueConverter* temp = new IntegerToCurrencyConverter();
        auto converter = shared_ptr<IValueConverter>(temp);
        cout << employee->toString() << ", salary: " << converter->convert(&integer) << endl;
    }
}